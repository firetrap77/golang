...
func readConfig() string {
    ...
    fmt.Println(configuration.Webhook) // output: [UserA, UserB]
...
func postToSlack(selection string, record AutoGenerated,webhookUrl string) {
...
title := record.Query.Search[index].Title
clean_title := strings.Replace(title, " ", "_", -1)
fullUrl := fmt.Sprintf("https://en.wikipedia.org/wiki/%s",clean_title)
...
//    attachment1.AddField(slack.Field { Title: "Author", Value: "Ashwanth Kumar" }).AddField(slack.Field { Title: "Status", Value: "Completed" })
    ...
    slack_err := slack.Send(webhookUrl, "", payload)
    if len(slack_err) > 0 {
      fmt.Printf("error: %s\n", slack_err)
    }
...
func main() {
    ...
    postToSlack(selection, record, webhookUrl)
}
